...
- ~~`HYBRID`에서 미승격 이상행동을 `/summary`에 이미 노출한 뒤 같은 날 나중에 승격되면 중복 노출을 어떻게 다룰지~~
  **(§4.5에서 해소, 2026-09)**: 중복 노출을 **허용**한다. `/summary`(`/report/weekly` 포함)는 알림 모드·일시정지와
  무관하게 기간에 겹치는 이상행동을 항상 전부 노출하므로 "미승격만 리포트에 노출"이라는 전제 자체가 없어졌다.
  리포트는 그 시점의 스냅샷이고 승격 푸시는 "임계 시간이 지나도 해소되지 않았다"는 상태 변화라 서로 다른 정보이며,
  다중 경로 확인의 안전 이점이 알림 피로 비용보다 크다. 승격을 억제하면 §15.6(Guardian별 개별 승격 판단)과
  충돌하고 미해소 상황의 알림이 사라질 수 있어(fail-open) 채택하지 않았다. 대신 응답 항목의 `notified`
  (요청 Guardian이 승격 푸시를 실제로 받았는가, `NotificationHistory` 기준, `FAILED`는 `false`)로만 구분한다.
  응답 구조는 API_Specification.md §3.6 참고. 기존 `AnomalyEventRepository`의 `...EscalatedAtIsNull` 조회는
  `escalated_at`이 "최초 승격 시각(참고용)"이라 Guardian별 판별에 쓸 수 없어 삭제했다.
