...
│   ├── logging.md      # 완료
│   ├── cache.md        # 완료
│   └── collaboration.md # 완료 — AI 협업 절차(Trade-off 제시, 디버깅, 코드 리뷰)
│
...
## AI 협업 원칙
- AI(Claude Code)는 구현을 보조하는 도구이며, 요구사항 정의·아키텍처 및 기술 선택·최종
  검증은 사용자가 담당한다.
- 불명확한 요구사항을 임의로 해석해 구현하지 않는다 — 범위나 의도가 애매하면 먼저 사용자에게
  확인한다.
- 새로운 기술적 결정(DB 구조/Index, Cache, Security, Transaction/Async, API, Architecture,
  외부 라이브러리, Infra 등)이 필요한 경우 코드부터 작성하지 않고 선택지와 Trade-off를 먼저
  제시한다. 이미 `docs/`에 확정되어 있는 결정은 임의로 재검토하지 않고 그대로 따른다.
- 구체적인 절차(Trade-off 제시가 필요한 상황, 디버깅 순서, 코드 리뷰 기준)는
  `.claude/rules/collaboration.md`를 따른다.
...
- 작업 완료 후 응답은 다음 순서로 정리한다.
  1. 무엇을 했는지 요약 (2~3줄)
  2. 핵심 설계/구현 방식을 선택한 이유 (대안이 있었다면 왜 배제했는지 1~2줄)
  3. 변경된 파일 목록
  4. 실행/테스트 방법 (해당되는 경우)
  5. 남은 이슈나 확인이 필요한 사항
